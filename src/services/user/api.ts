...
export const removeUsers = async (query: ApiQuery) => {
  const response = await Fetch.Delete<any>(userApiPaths.remove + getApiQuery(query), 'removeUsers');
  return response;
};

export const removeAddress = async (query: ApiQuery) => {
  const response = await Fetch.Delete<any>(userApiPaths.removeAddress + getApiQuery(query), 'removeAddress');
  return response;
};
